    ...
    <div className="card">
      {!authorized ? (
        <div>
          ...
          <p className="small">Enter admin code to manage devices (change in production).</p>
          <input className="input" value={code} onChange={e=>setCode(e.target.value)} placeholder="Admin code" />
          <div style={{marginTop:8}}><button className="btn" onClick={unlock}>Unlock</button></div>
        </div>
      ):(
        <div>
          <h3>Devices</h3>
          ...
          <div style={{marginTop:8}}><button className="btn" onClick={addDevice}>Add Device</button></div>
        </div>
      )}
    </div>
  )
